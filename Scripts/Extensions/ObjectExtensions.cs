...
{
    static readonly Dictionary<Delegate, object> cache = new Dictionary<Delegate, object>();
    static readonly Dictionary<Object, List<Delegate>> subs = new Dictionary<Object, List<Delegate>>();
    public static T Lazy<T>(Func<T> factory)
    {
        // performance can be improved...
        // https://www.youtube.com/watch?v=jblRYDMTtvg
        if (!cache.ContainsKey(factory))
            cache.Add(factory, factory());

        return (T)cache[factory];
    }
    public static T Lazy<T>(this Component obj, Func<T> factory)
    {
        if (!subs.ContainsKey(obj))
            subs.Add(obj, new List<Delegate>());


        void RemoveFromCache()
        {
            foreach (var d in subs[obj])
                cache.Remove(d);
            subs.Remove(obj);
        }

        obj?.gameObject?.OnDestroyAddListener(RemoveFromCache);
