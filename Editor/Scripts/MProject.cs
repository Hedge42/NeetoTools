    ...
    private static void OnProjectWindowGUI(string guid, Rect rect)
    {
        ...
        if (isText || isObject || isFolder)
        {
            ...
            if (isText)
            {
                ...
                content.image = MTexture.click;
            }
            else if (isFolder)
            {
                content.tooltip = "Open in file explorer";
                content.image = MTexture.folder;
            }
            else if (isObject)
            {
                content.tooltip = "Inspect";
                content.image = MTexture.inspect;
            }

            if (isObject)
            {

                if (GUI.Button(rect, content, Styles.iconButton))
                {
            ...
            else if (isText && GUI.Button(rect, content, Styles.iconButton))
            {
            ...
            else if (isFolder && GUI.Button(rect, content, Styles.iconButton))
            {
                var assetPath = path;
                ...
                path = Path.Combine(MPath.project, path);
                path = path.Replace("/", "\\");
