...
namespace Matchwork
{
    ...
    {
        private const string WINDOW_TITLE = "Prefab Replacer";
        private const string MENU_ITEM = MPath.Main + WINDOW_TITLE;
        ...
        [MenuItem(MENU_ITEM, priority = MPath.BOTTOM)]
        public static PrefabEditorWindow Open()
        {
            var window = GetWindow<PrefabEditorWindow>(false, WINDOW_TITLE, true);
            window.Show();
            return window;
        }
        ...
        [SerializeField, SerializeReference, Polymorphic(
            refTypes: new[] { typeof(ITransformOperation), typeof(PrefabOverride)
        })]
